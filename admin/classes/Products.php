...
class Products
{
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/KhanStore/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Sản phẩm đã được thêm...!'];
					}else{
				...
				return ['status'=> 303, 'message'=> 'Ảnh có kích thước quá lớn...!'];
			}

		}else{
			return ['status'=> 303, 'message'=> 'Định dạng không hợp lê...! [ jpg, jpeg, png]'];
		}
		...
		if ($ext == "jpg" || $ext == "jpeg" || $ext == "png") {
			...
			if ($file['size'] > (1024 * 2)) {
				...
				if (move_uploaded_file($file['tmp_name'], $_SERVER['DOCUMENT_ROOT']."/KhanStore/product_images/".$uniqueImageName)) {
					...
					if ($q) {
						return ['status'=> 202, 'message'=> 'Sản phẩm đã được cập nhật...!'];
					}else{
						return ['status'=> 303, 'message'=> 'Lỗi...!'];
					}
	...
	public function editProductWithoutImage($pid,
		...
		if ($pid != null) {
			...
			return ['status'=> 303, 'message'=> 'Không hợp lệ...!'];
		}
	...
	public function addCategory($name){
		...
		if ($q->num_rows > 0) {
			return ['status'=> 303, 'message'=> 'Danh mục đã tồn tại...!'];
		}else{
			$q = $this->con->query("INSERT INTO categories (cat_title) VALUES ('$name')");
			if ($q) {
				return ['status'=> 202, 'message'=> 'Danh mục đã được thêm...!'];
			}else{
				return ['status'=> 303, 'message'=> 'Lỗi...!'];
			}
		}
	...
	public function deleteProduct($pid = null){
		if ($pid != null) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Xóa thành công...!'];
			}else{
				return ['status'=> 202, 'message'=> 'Lỗi...!'];
			}
	...
	public function updateCategory($post = null){
		...
		if (!empty($cat_id) && !empty($e_cat_title)) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Cập nhật thành công...!'];
			}else{
				return ['status'=> 202, 'message'=> 'Lỗi...!'];
			}
			
		}else{
			return ['status'=> 303, 'message'=>'Không hợp lê...!'];
		}
	...
	public function addBrand($name){
		...
		if ($q->num_rows > 0) {
			...
			if ($q) {
				return ['status'=> 202, 'message'=> 'Hãng đã được thêm...!'];
			}else{
				return ['status'=> 303, 'message'=> 'Lỗi...!'];
			}
		}
...
if (isset($_POST['add_category'])) {
	if (isset($_SESSION['admin_id'])) {
		...
		echo json_encode(['status'=> 303, 'message'=> 'Error']);
	}
}
